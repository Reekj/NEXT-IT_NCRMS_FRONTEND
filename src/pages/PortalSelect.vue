<template>
  ...
  >
    ...
    <div class="relative mx-auto w-full max-w-[1280px] px-6 py-10 sm:py-14">
      <!-- TOP BAR -->
      <header class="flex items-center justify-between gap-4">
        ...
        <button type="button" class="chipBtn" @click="goSupport">
      ...
      <section class="mt-10 sm:mt-12">
        <div class="hero">
          ...
          <p class="hero__sub">
            Sign in through the appropriate authorized agency domain. Access is monitored, audited,
            and protected with strict security controls.
          </p>
      ...
      <section class="mt-10 sm:mt-12">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          <article
            ...
            :key="portal.title"
            class="portalCard"
            :class="portal.variant === 'primary' ? 'portalCard--primary' : 'portalCard--neutral'"
            @mousemove="onCardMove"
            @mouseleave="resetCard"
          >
            <div
              class="portalCard__topline"
              :class="portal.variant === 'primary' ? '' : 'portalCard__topline--neutral'"
              aria-hidden="true"
            />

            <div class="portalCard__inner">
              <div
                class="portalCard__icon"
                :class="portal.variant === 'primary' ? '' : 'portalCard__icon--neutral'"
              >
                <component :is="portal.icon" class="h-5 w-5 text-white" />
              </div>

              <div class="portalCard__content">
                <div class="portalCard__headingRow">
                  <h2 class="portalCard__title">{{ portal.title }}</h2>

                  <span
                    class="tag"
                    :class="portal.variant === 'primary' ? 'tag--blue' : 'tag--slate'"
                  >
                    {{ portal.tag }}
                  </span>
                </div>
                ...
                <p class="portalCard__desc">
                  {{ portal.description }}
                  <span v-if="portal.comingSoon" class="ml-1 font-semibold text-white/80">
                    (Coming soon)
                  </span>
                </p>
                ...
                <div class="portalCard__actions">
                  <button
                    type="button"
                    :class="portal.variant === 'primary' ? 'btnPrimary' : 'btnGhost'"
                    @click="goPortal(portal.route)"
                    @mouseenter="portal.variant === 'primary' ? primeMagnet($event, true) : null"
                    @mouseleave="portal.variant === 'primary' ? primeMagnet($event, false) : null"
                    @mousemove="portal.variant === 'primary' ? onMagnetMove($event) : null"
                  >
                    <span
                      v-if="portal.variant === 'primary'"
                      class="btnPrimary__shine"
                      aria-hidden="true"
                    />
                    Continue
            ...
            <div
              class="portalCard__borderGlow"
              :class="portal.variant === 'primary' ? '' : 'portalCard__borderGlow--neutral'"
              aria-hidden="true"
            />
          </article>
        </div>
...
const portals = [
  {
    ...
    title: "Law Enforcement NPF",
    tag: "Police • Oversight • Operations",
    description:
    ...
    route: "/roles",
    variant: "primary",
    microOne: "Fast role access",
    microTwo: "Audit trails",
  },
  {
    icon: markRaw(Building2),
    title: "Ministry of Interior",
    tag: "Policy • Governance • Oversight",
    description:
      "Access ministry-level oversight tools, criminal record coordination, approval workflows, and national security administration modules.",
    route: "/",
    variant: "neutral",
    microOne: "Executive access",
    microTwo: "Governance logs",
    comingSoon: true,
  },
  {
    icon: markRaw(Car),
    title: "Federal Road Safety Corps",
    tag: "Traffic • Licensing • Enforcement",
    description:
      "Access road safety enforcement workflows, offender verification, driver-related record checks, and inter-agency mobility intelligence.",
    route: "/",
    variant: "neutral",
    microOne: "Identity checks",
    microTwo: "Traffic records",
    comingSoon: true,
  },
  {
    icon: markRaw(Shield),
    title: "Correctional Service",
    tag: "Custody • Rehabilitation • Records",
    description:
      "Access correctional records, inmate verification, sentence history, release coordination, and secure institutional reporting tools.",
    route: "/",
    variant: "neutral",
    microOne: "Custody records",
    microTwo: "Release logs",
    comingSoon: true,
  },
  {
    icon: markRaw(Plane),
    title: "Immigration Service",
    tag: "Border • Identity • Movement",
    description:
      "Access immigration-related identity checks, travel movement validation, watchlist coordination, and border security workflows.",
    route: "/",
    variant: "neutral",
    microOne: "Border checks",
    microTwo: "Movement logs",
    comingSoon: true,
  },
  {
    icon: markRaw(Landmark),
    title: "NSA Office",
    tag: "National Security • Intelligence • Risk",
    description:
      "Access high-security coordination modules for intelligence review, threat-linked record validation, and national security oversight.",
    route: "/",
    variant: "neutral",
    microOne: "Risk review",
    microTwo: "Secure audit",
    comingSoon: true,
  },
  {
    icon: markRaw(ShieldCheck),
    title: "Homeland Security",
    tag: "Protection • Threat Response • Coordination",
    description:
      "Access homeland security workflows for verified records, threat coordination, protected access, and multi-agency response intelligence.",
    route: "/",
    variant: "neutral",
    microOne: "Threat checks",
    microTwo: "Response logs",
    comingSoon: true,
  },
  {
    icon: markRaw(BriefcaseBusiness),
    title: "Authorized Civil Service Agencies and Employment Bodies",
    tag: "Employment • Clearance • Verification",
    description:
      "Access authorized employment verification workflows, civil service clearance checks, background screening, and controlled record requests.",
    route: "/",
    variant: "neutral",
    microOne: "Clearance checks",
    microTwo: "Request logs",
    comingSoon: true,
  },
];
...
.fx__mesh {
  ...
  inset: -2px;
  background:
    radial-gradient(900px 700px at 20% 20%, rgba(32, 92, 255, 0.4), transparent 60%),
...
.fx__spotlight {
  ...
  background:
    radial-gradient(
      520px 520px at var(--mx, 50%) var(--my, 20%),
...
.hero__titleGlow {
  position: absolute;
  inset: -20px -10px;
  background: radial-gradient(520px 200px at 30% 20%, rgba(80, 160, 255, 0.2), transparent 60%);
  filter: blur(8px);
  pointer-events: none;
}
...
.chipBtn__ring {
  ...
  inset: -2px;
  border-radius: 9999px;
  background: radial-gradient(circle at 20% 30%, rgba(80, 160, 255, 0.3), transparent 55%);
  opacity: 0;
  transition: opacity 0.25s ease;
  pointer-events: none;
}

.chipBtn:hover .chipBtn__ring {
  opacity: 1;
}
...
.portalCard__borderGlow {
  ...
  border-radius: 26px;
  background: radial-gradient(
    260px 180px at var(--sx, 30%) var(--sy, 30%),
...
.portalCard.is-active .portalCard__borderGlow {
  opacity: 1;
}
...
.notice__icon {
  ...
  display: grid;
  place-items: center;
  background: linear-gradient(180deg, rgba(80, 160, 255, 0.3), rgba(10, 35, 149, 0.1));
