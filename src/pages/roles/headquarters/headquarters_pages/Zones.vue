...
<template>
    <HeadquartersLayout :user="user">
      ...
      >
        ...
        >
          <!-- Header controls (search + refresh left, New right) -->
          <div
  ...
  onMounted(() => {
    ...
    if (!rows.value.length) {
      const demo = Array.from({ length: 9 }).map((_, i) => {
        return {
          id: crypto?.randomUUID ? crypto.randomUUID() : `zone-demo-${i + 1}`,
          zoneName: `Zone ${i + 1}`,
  ...
  watch(
    () => route.fullPath,
    () => {
      if (route.path === "/headquarters/zones") syncRows();
    }
  );
  ...
  <style scoped>
  /* IMPORTANT: vertical separators (border-right) on every column, like Figma */
  .thCell {
    height: 58px;
    text-align: left;
    padding: 0 18px;
