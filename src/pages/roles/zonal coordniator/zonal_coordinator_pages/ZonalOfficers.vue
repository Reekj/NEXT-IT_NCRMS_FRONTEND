<!-- src/pages/roles/zonal coordniator/zonal_coordinator_pages/ZonalOfficers.vue -->
<template>
    <ZonalCoordinatorLayout>
      ...
      >
        ...
        <div class="px-6 pt-7 sm:px-8">
          <h1 class="text-[22px] font-semibold text-[#111827]">Zonal Officers</h1>
        </div>
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div>
                ...
                <div class="mt-1 text-[12px] text-[#667085]">{{ row.badgeId }} • {{ row.assignedState }}</div>
              </div>
            </div>
  
            <div class="mt-4 grid grid-cols-2 gap-3 text-[12px]">
              <div class="col-span-2">
                <div class="text-[#667085]">Permission</div>
                <div class="font-medium text-[#111827]">{{ permissionLabel(row.permissions) }}</div>
              </div>
              <div>
              ...
              <div>
                <div class="text-[#667085]">Phone</div>
        ...
        <!-- Pagination (static UI like your pattern) -->
  ...
  const headers = ["Full Name", "Badge ID", "Assigned State", "Permission", "Date Created", "Actions"]
    ...
    const seed = [
      {
        ...
        badgeId: "ZO-001",
        assignedState: "Oyo State",
        phone: "07020100100",
        dateCreated: "2025-01-12",
        permissions: {
          staffDirectoryView: true,
          staffDirectoryAddEdit: false,
          staffReportView: true,
          staffReportGenerate: false,
          criminalRecordView: true,
          criminalRecordGenerate: false,
          incidentReportView: true,
  ...
  const filteredRows = computed(() => {
    ...
    return rows.value.filter((r) => {
      return (
        ...
        String(r.badgeId || "").toLowerCase().includes(s) ||
        String(r.assignedState || "").toLowerCase().includes(s) ||
        permissionLabel(r.permissions).toLowerCase().includes(s) ||
  ...
  function permissionLabel(p) {
    const perms = p || {}
    const hasAddEdit = !!perms.staffDirectoryAddEdit
    const hasGenerate = !!perms.staffReportGenerate || !!perms.criminalRecordGenerate || !!perms.incidentReportGenerate
    // If ONLY view permissions are on (no add/edit, no generate) => View only
    if (!hasAddEdit && !hasGenerate) return "View only"
    return "Limited"
  }
  ...
  function goNew() {
    router.push({ name: "zc-zonal-officers-create" })
  }
  function goView(id) {
    router.push({ name: "zc-zonal-officers-view", params: { id } })
  }
  function goEdit(id) {
