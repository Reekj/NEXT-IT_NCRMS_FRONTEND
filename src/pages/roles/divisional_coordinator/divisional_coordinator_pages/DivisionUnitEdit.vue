<template>
    <DivisionalCoordinatorLayout :user="user">
      <div class="mb-6 text-[18px] font-semibold text-[#111827]">Surulere Division</div>
  
      <Motion
        :initial="{ opacity: 0, y: 10 }"
        :animate="{ opacity: 1, y: 0 }"
        :transition="{ duration: 0.35 }"
        class="rounded-2xl border border-black/10 bg-white shadow-[0_8px_24px_rgba(16,24,40,0.08)]"
      >
        <div class="border-b border-black/10 px-6 py-5 sm:px-8">
          <h2 class="text-[18px] font-semibold text-[#111827]">Edit Division Unit</h2>
        </div>
  
        <div v-if="loaded" class="px-6 py-7 sm:px-10 sm:py-10">
          <div class="grid grid-cols-1 gap-x-14 gap-y-10 md:grid-cols-2">
            <div>
              <label class="mb-3 block text-[14px] font-medium text-[#111827]">Unit Name</label>
              <input
                v-model="model.unitName"
                type="text"
                class="h-14 w-full rounded-2xl border border-[#D7E3FF] bg-white px-5 text-[14px] text-[#6E88C7]
                       outline-none transition focus:border-[#0A2395] focus:ring-4 focus:ring-[#0A2395]/10"
              />
            </div>
  
            <div>
              <label class="mb-3 block text-[14px] font-medium text-[#111827]">Unit Type</label>
              <input
                v-model="model.unitType"
                type="text"
                class="h-14 w-full rounded-2xl border border-[#D7E3FF] bg-white px-5 text-[14px] text-[#6E88C7]
                       outline-none transition focus:border-[#0A2395] focus:ring-4 focus:ring-[#0A2395]/10"
              />
            </div>
  
            <div>
              <label class="mb-3 block text-[14px] font-medium text-[#111827]">Total Officers</label>
              <div class="relative">
                <select
                  v-model="model.totalOfficers"
                  class="h-14 w-full appearance-none rounded-2xl border border-[#D7E3FF] bg-white px-5 pr-12 text-[14px] text-[#6E88C7]
                         outline-none transition focus:border-[#0A2395] focus:ring-4 focus:ring-[#0A2395]/10"
                >
                  <option value="">Select...</option>
                  <option v-for="n in officersOptions" :key="n" :value="String(n)">{{ n }}</option>
                </select>
                <ChevronDown class="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#667085]" />
              </div>
            </div>
  
            <div>
              <label class="mb-3 block text-[14px] font-medium text-[#111827]">Unit Head</label>
              <div class="relative">
                <select
                  v-model="model.unitHead"
                  class="h-14 w-full appearance-none rounded-2xl border border-[#D7E3FF] bg-white px-5 pr-12 text-[14px] text-[#6E88C7]
                         outline-none transition focus:border-[#0A2395] focus:ring-4 focus:ring-[#0A2395]/10"
                >
                  <option value="">Select...</option>
                  <option v-for="h in heads" :key="h" :value="h">{{ h }}</option>
                </select>
                <ChevronDown class="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#667085]" />
              </div>
            </div>
  
            <div>
              <label class="mb-3 block text-[14px] font-medium text-[#111827]">Status</label>
              <div class="relative">
                <select
                  v-model="model.status"
                  class="h-14 w-full appearance-none rounded-2xl border border-[#D7E3FF] bg-white px-5 pr-12 text-[14px] text-[#6E88C7]
                         outline-none transition focus:border-[#0A2395] focus:ring-4 focus:ring-[#0A2395]/10"
                >
                  <option value="">Select...</option>
                  <option value="Active">Active</option>
                  <option value="Inactive">Inactive</option>
                </select>
                <ChevronDown class="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#667085]" />
              </div>
            </div>
  
            <div>
              <label class="mb-3 block text-[14px] font-medium text-[#111827]">Date Created</label>
              <div class="relative">
                <input
                  v-model="model.dateCreated"
                  type="date"
                  class="h-14 w-full rounded-2xl border border-[#D7E3FF] bg-white px-5 pr-12 text-[14px] text-[#6E88C7]
                         outline-none transition focus:border-[#0A2395] focus:ring-4 focus:ring-[#0A2395]/10"
                />
                <Calendar class="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#667085]" />
              </div>
            </div>
          </div>
  
          <div class="mt-10 flex flex-wrap items-center gap-4">
            <Motion
              as="button"
              type="button"
              :whileHover="{ scale: 1.02 }"
              :whileTap="{ scale: 0.98 }"
              @click="onUpdate"
              class="inline-flex h-11 items-center justify-center rounded-xl px-8 text-[14px] font-semibold text-white
                     bg-[linear-gradient(90deg,#0A2395_0%,#030B2F_100%)] shadow-sm"
            >
              Update
            </Motion>
  
            <Motion
              as="button"
              type="button"
              :whileHover="{ scale: 1.02 }"
              :whileTap="{ scale: 0.98 }"
              @click="onCancel"
              class="inline-flex h-11 items-center justify-center rounded-xl border border-black/10 bg-white px-8 text-[14px] font-semibold text-[#111827]
                     shadow-sm hover:bg-[#F9FAFB]"
            >
              Cancel
            </Motion>
          </div>
        </div>
  
        <div v-else class="px-6 py-10 sm:px-10">
          <div class="text-[13px] text-black/50">Loading...</div>
        </div>
      </Motion>
    </DivisionalCoordinatorLayout>
  </template>
  
  <script setup>
  import { onMounted, ref } from "vue"
  import { useRoute, useRouter } from "vue-router"
  import { Motion } from "@motionone/vue"
  import { Calendar, ChevronDown } from "lucide-vue-next"
  import DivisionalCoordinatorLayout from "../layout/Layout.vue"
  
  /** inline storage (same as main table/create/view) */
  const UNITS_KEY = "ncrms_dc_division_units"
  const UNITS_EVENT = "ncrms-dc-division-units-updated"
  
  function safeRead() {
    try {
      const raw = localStorage.getItem(UNITS_KEY)
      const parsed = raw ? JSON.parse(raw) : []
      return Array.isArray(parsed) ? parsed : []
    } catch {
      return []
    }
  }
  
  function safeWrite(next) {
    localStorage.setItem(UNITS_KEY, JSON.stringify(Array.isArray(next) ? next : []))
    window.dispatchEvent(new CustomEvent(UNITS_EVENT))
  }
  
  const router = useRouter()
  const route = useRoute()
  
  const user = ref({ name: "Helena John", role: "Divisional Coordinator", avatarUrl: "" })
  const officersOptions = [4, 8, 9, 12, 14, 16, 20, 22, 30]
  const heads = ["Micheal Jackson", "A. Kehinde", "O. Ibrahim", "S. Okafor"]
  
  const loaded = ref(false)
  const model = ref({
    id: "",
    unitName: "",
    unitType: "",
    totalOfficers: "",
    unitHead: "",
    dateCreated: "",
    status: "",
  })
  
  onMounted(() => {
    const id = String(route.params.id || "")
    const list = safeRead()
    const found = list.find((x) => String(x.id) === id)
  
    if (!found) {
      router.push({ path: "/divisional-coordinator/division-units" })
      return
    }
  
    model.value = { ...found }
    loaded.value = true
  })
  
  function onUpdate() {
    const list = safeRead()
    const idx = list.findIndex((x) => x.id === model.value.id)
    if (idx === -1) return
  
    const next = [...list]
    next[idx] = { ...next[idx], ...model.value }
  
    safeWrite(next)
    router.push({ path: "/divisional-coordinator/division-units" })
  }
  
  function onCancel() {
    router.push({ path: "/divisional-coordinator/division-units" })
  }
  </script>
  