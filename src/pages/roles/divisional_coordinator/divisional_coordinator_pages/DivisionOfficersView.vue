<template>
    <DivisionalCoordinatorLayout :user="user">
      ...
      >
        <div class="border-b border-black/10 px-6 py-5 sm:px-8">
          <h2 class="text-[18px] font-semibold text-[#111827]">View Division Officer</h2>
        </div>
        ...
        <div v-if="row" class="px-6 py-7 sm:px-10 sm:py-10">
          <div class="grid grid-cols-1 gap-x-14 gap-y-10 md:grid-cols-2">
            ...
            <Field label="Division ID" :value="row.divisionId" />
            <Field label="Rank" :value="row.rank" />
  ...
  onMounted(() => {
    ...
    if (!row.value) router.push({ path: "/divisional-coordinator/division-officers" })
  })
  ...
  function goEdit() {
    if (!row.value) return
    router.push({ path: `/divisional-coordinator/division-officers/${row.value.id}/edit` })
  }
